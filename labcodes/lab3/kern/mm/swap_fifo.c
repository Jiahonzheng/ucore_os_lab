...
static int _fifo_map_swappable(struct mm_struct *mm, uintptr_t addr,
  ...
  list_entry_t *head = (list_entry_t *)mm->sm_priv;  // 获取调页队列的起始地址
  list_entry_t *entry = &(page->pra_page_link);  // 得到调入页的地址
...
static int _fifo_swap_out_victim(struct mm_struct *mm, struct Page **ptr_page,
  ...
  list_entry_t *le = head->prev;  // 获取调出页
  assert(head != le);
  struct Page *p = le2page(le, pra_page_link);  // 获取调出页的物理地址
  //将最早添加的内存页从 pra_list_head 队列移除
  list_del(le);
  assert(p != NULL);
  // 为 *ptr_page 赋值
  *ptr_page = p;
