...
static struct proc_struct *alloc_proc(void) {
  ...
  if (proc != NULL) {
    ...
    proc->state = PROC_UNINIT;  // 设置进程状态
    proc->pid = -1;             // 初始化 pid 为 -1
    proc->runs = 0;             // 初始化进程运行次数为 0
    proc->kstack = 0;           // 初始化内核堆栈为 0
    proc->need_resched = 0;     // 初始化为不需要调度
    proc->parent = NULL;        // 初始化父进程为空
    proc->mm = NULL;            // 初始化内存控制块为空
    memset(&(proc->context), 0, sizeof(struct context));  // 设置进程上下文为空
    proc->tf = NULL;  // 设置中断陷入帧为空
    proc->cr3 =
        boot_cr3;  // 内核态线程不需要分配新的页目录表地址，直接使用内核页表
    proc->flags = 0;
    memset(proc->name, 0, PROC_NAME_LEN);
    proc->wait_state = 0;  // 初始化等待态
    // 初始化 cptr optr 和 yptr
    proc->cptr = NULL;
    proc->yptr = NULL;
    proc->optr = NULL;
    /*
     ...
     * Process list_entry_t run_link;                      // the entry linked
     * in run queue int time_slice;                             // time slice
     * for occupying the CPU skew_heap_entry_t lab6_run_pool;            // FOR
     * LAB6 ONLY: the entry in the run pool uint32_t lab6_stride; // FOR LAB6
     * ONLY: the current stride of the process uint32_t lab6_priority; // FOR
     * LAB6 ONLY: the priority of process, set by lab6_set_priority(uint32_t)
     */
    proc->rq = NULL;               // 初始化进程所需就绪队列
    list_init(&(proc->run_link));  // 初始化就绪队列指针
    proc->time_slice = 0;          // 初始化时间片为 0
    proc->lab6_run_pool.left = NULL;
    proc->lab6_run_pool.right = NULL;
    proc->lab6_run_pool.parent = NULL;
    proc->lab6_stride = 0;    // 初始化 Stride
    proc->lab6_priority = 0;  // 初始化 Priority
...
// decrease the ref_count of files, and if ref_count==0, then destroy
// files_struct
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
  ...
  {
    proc->pid = get_pid();  // 设置进程 PID
    // nr_process++;                              // 全部进程数+1
    // list_add(&proc_list, &(proc->list_link));  // 将其加入到进程链表
    hash_proc(proc);  // 将其加入至进程 hash 链表
    set_links(proc);
  }
...
static int load_icode(int fd, int argc, char **kargv) {
   ...
   * process's memory? how to setup argc/argv? MACROs or Functions: mm_create -
   * create a mm setup_pgdir      - setup pgdir in mm load_icode_read  - read
   * raw data content of program file mm_map           - build new vma
   ...
   * elfhdr (3.3) call mm_map to build vma related to TEXT/DATA (3.4)
   ...
   * these pages (4) call mm_map to setup user stack, and put parameters into
   * user stack (5) setup current process's mm, cr3, reset pgidr (using lcr3
   * MARCO) (6) setup uargc and uargv in user stacks (7) setup trapframe for
   * user environment (8) if up steps failed, you should cleanup the env.
   */
...
static int copy_kargv(struct mm_struct *mm, int argc, char **kargv,
                      const char **argv) {
  int i, ret = -E_INVAL;
...
int do_execve(const char *name, int argc, const char **argv) {
  ...
  /* sysfile_open will check the first argument path, thus we have to use a
   * user-space pointer, and argv[0] may be incorrect */
  int fd;
