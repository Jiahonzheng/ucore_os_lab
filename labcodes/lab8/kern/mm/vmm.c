...
int do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
  ...
  if (*ptep == 0) {
    ...
    if (*ptep & PTE_P) {
      // if process write to this existed readonly page (PTE_P means existed),
      // then should be here now. we can implement the delayed memory space copy
      // for fork child process (AKA copy on write, COW). we didn't implement
      // now, we will do it in future.
