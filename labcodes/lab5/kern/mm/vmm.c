...
int do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
  ...
  switch (error_code & 3) {
    ...
    case 2: /* error code flag : (W/R=1, P=0): write, not present */
      if (!(vma->vm_flags & VM_WRITE)) {
        cprintf(
            "do_pgfault failed: error code flag = write AND not present, but "
    ...
    case 0: /* error code flag : (W/R=0, P=0): read, not present */
      if (!(vma->vm_flags & (VM_READ | VM_EXEC))) {
        cprintf(
            "do_pgfault failed: error code flag = read AND not present, but "
  ...
  if (ptep == NULL) {  // 若页表项获取失败
    cprintf("do_pgfault failed: fail to get pte\n");
    goto failed;
  }
  // if the phy addr isn't exist, then alloc a page & map the
  // phy addr with logical addr
  if (*ptep == 0) {
    // 若页表项不存在，则分配空闲页，建立物理地址与逻辑地址的对应关系
    if (pgdir_alloc_page(mm->pgdir, addr, perm) == NULL) {
      cprintf("do_pgfault failed: failed to execute pgdir_alloc_page\n");
      goto failed;
    }
  } else {
    ...
    // logical addr
    if (swap_init_ok) {  // 如果允许交换页面
      struct Page *page = NULL;
      if ((ret = swap_in(mm, addr, &page)) != 0) {  // 将对应物理空间调入内存
        cprintf("swap_in in do_pgfault failed\n");
        goto failed;
      }
      page_insert(mm->pgdir, page, addr,
                  perm);  // 建立虚拟地址和物理地址的对应关系
      swap_map_swappable(mm, addr, page,
                         1);  // 设置此内存页为可交换，更新 sm_priv 队列
      page->pra_vaddr = addr;
    } else {
      cprintf("no swap_init_ok but ptep is %x, failed\n", *ptep);
      goto failed;
