...
void pmm_init(void) {
  ...
  // the TSS map virtual_addr 0 ~ 4G = linear_addr 0 ~ 4G then set kernel stack
  // (ss:esp) in TSS, setup TSS in gdt, load TSS
  gdt_init();
...
/* *
 * get_pie get pte and return the kernel virtual address of this pte for la if
 * the PT contians this pte didn't exist, alloc a page for PT
...
/* *
 * page_remove_pte free an Page struct which is related linear address la and
  ...
  if (page != NULL) {
    ...
    if (swap_init_ok) {
      if (check_mm_struct != NULL) {
        ...
        // page->pra_vaddr,page->pra_page_link.prev, page->pra_page_link.next);
      } else {  // now current is existed, should fix it in the future
        // swap_map_swappable(current->mm, la, page, 0);
        // page->pra_vaddr=la;
        // assert(page_ref(page) == 1);
        // panic("pgdir_alloc_page: no pages. now current is existed, should fix
        // it in the future\n");
      }
    }
  }
