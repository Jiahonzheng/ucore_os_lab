...
int debuginfo_eip(uintptr_t addr, struct eipdebuginfo *info) {
  ...
  if (addr >= KERNBASE) {
    ...
    // program's stabs (included __STAB_BEGIN__, __STAB_END__,
    // __STABSTR_BEGIN__, and __STABSTR_END__) in a structure located at virtual
    // address USTAB.
    ...
    if (!user_mem_check(mm, (uintptr_t)stabs,
...
/**
 * read_eip - read the value of current eip, since while calling this function,
 * it can read the caller's eip from stack easily.
 */
 ...
 * the boundary.
 *
 * 栈顶在内存布局的最低处，栈底在内存布局的最高处。
 *
 * 堆栈指针寄存器 esp 指向栈顶。向堆栈压入元素，将导致 esp 寄存器值减小。
 *
 * 基址指针寄存器 ebp 指向栈底。当进入一个函数时，会向栈中压入调用者的 ebp 和
 * eip 寄存器值， 同时将 ebp 设置为之前的 esp 值，即 ebp 寄存器保存的是调用者的
 * ebp 地址。
 *
 * 如果所有的函数都遵循上述的约定，则我们可以利用此约定追踪程序的调用堆栈。
 * */
void print_stackframe(void) {
  ...
  for (i = 0; ebp && i < STACKFRAME_DEPTH; ++i) {
    ...
    // (3.5) popup a calling stackframe NOTICE: the calling funciton's return
    // addr eip  = ss:[ebp+4] the calling funciton's ebp = ss:[ebp]
    eip = *((uint32_t *)ebp + 1);  // 设置 eip 为调用者 eip
    ebp = *((uint32_t *)ebp);      // 设置 ebp 为调用者 ebp
  }
}
