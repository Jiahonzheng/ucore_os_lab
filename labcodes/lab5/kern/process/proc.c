...
static struct proc_struct *alloc_proc(void) {
  ...
  if (proc != NULL) {
    ...
    proc->cptr = proc->yptr = proc->optr = NULL;  // 初始化 cptr optr 和 yptr
  }
  return proc;
}
...
static void set_links(struct proc_struct *proc) {
  list_add(&proc_list, &(proc->list_link));  // 添加进程至进程列表
  // 设置新旧进程的关系
  proc->yptr = NULL;
...
static void remove_links(struct proc_struct *proc) {
  list_del(&(proc->list_link));  // 从进程列表移除进程
  // 设置新旧进程的关系
  if (proc->optr != NULL) {
...
int do_execve(const char *name, size_t len, unsigned char *binary,
              size_t size) {
  struct mm_struct *mm = current->mm;
