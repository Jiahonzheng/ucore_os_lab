 ...
 * size >=n, then return NULL. (5) `default_free_pages`: re-link the pages into
 * the free list, and may merge small free blocks into the big ones. (5.1)
 *      According to the base address of the withdrawed blocks, search the free
 *  list for its correct position (with address from low to high), and insert
 *  the pages. (May use `list_next`, `le2page`, `list_add_before`)
 *  (5.2)
 *      Reset the fields of the pages, such as `p->ref` and `p->flags`
 * (PageProperty) (5.3) Try to merge blocks at lower or higher addresses.
...
static void default_init_memmap(struct Page *base, size_t n) {
  ...
  for (; p != base + n; p++) {
    ...
    set_page_ref(p, 0);  // 页面都未被使用，因此引用数为 0
  }
  base->property = n;
