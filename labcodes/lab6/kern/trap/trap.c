...
void idt_init(void) {
   ...
   * kern/trap DIR) You can use  "extern uintptr_t __vectors[];" to define this
   * extern variable which will be used later. (2) Now you should setup the
   * entries of ISR in Interrupt Description Table (IDT). Can you see idt[256]
   * in this file? Yes, it's IDT! you can use SETGATE macro to setup each item
   * of IDT (3) After setup the contents of IDT, you will let CPU know where is
   * the IDT by using 'lidt' instruction. You don't know the meaning of this
  ...
  // user app to use syscall to get the service of ucore so you should setup the
  // syscall interrupt gate in here
  // 表示各个中断处理程序的段内偏移地址
...
static void trap_dispatch(struct trapframe *tf) {
  ...
  switch (tf->tf_trapno) {
    ...
    then you can add code here.
       ...
       *    Every TICK_NUM cycle, you should set current process's
      ...
      //   current->need_resched = 1;
      // }
      sched_class_proc_tick(current);  // 调用 proc_tick 函数
      break;
