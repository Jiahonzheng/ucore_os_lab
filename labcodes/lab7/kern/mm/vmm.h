...
struct mm_struct {
  list_entry_t mmap_list;  // 根据 vma 地址排序的链表
  struct vma_struct *mmap_cache;  // 当前访问的 vma 结构，用于缓存提升速度
  pde_t *pgdir;                   // vma 页目录表
  int map_count;                  // vma 数量
  void *sm_priv;                  // the private data for swap manager
  int mm_count;                   // 使用此 mm 的进程数量
  semaphore_t mm_sem;             // 信号量
...
static inline int mm_count(struct mm_struct *mm) { return mm->mm_count; }
