...
typedef struct {
  ...
  struct proc_struct *proc;  // 当前计时器绑定的进程
...
static inline timer_t *timer_init(timer_t *timer, struct proc_struct *proc,
                                  int expires) {
  timer->expires = expires;         // 设置过期时间
  timer->proc = proc;               // 将进程和定时器绑定
  list_init(&(timer->timer_link));  // 初始化定时器链表
  return timer;
}
...
void run_timer_list(
    void);  // call scheduler to update tick related info, and check the timer
            // is expired? If expired, then wakup proc
