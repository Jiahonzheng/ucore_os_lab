...
# philosophers and then declaring one's own intention to eat. This is a
two-phase # protocol; first declaring the status HUNGRY, then going on to EAT.
...
void phi_test_sema(i) {
  if (state_sema[i] == HUNGRY && state_sema[LEFT] != EATING &&
    ...
    state_sema[i] = EATING;  // 记录下第 i号哲学家进餐的事实
    up(&s[i]);
  }
}
...
void phi_take_forks_sema(int i) {
  down(&mutex);            // 进入临界区
  state_sema[i] = HUNGRY;  // 记录下第 i 号哲学家饥饿的事实
...
int philosopher_using_semaphore(void *arg) {
  ...
  while (iter++ < TIMES) {
    ...
    do_sleep(SLEEP_TIME);  // 模拟思考，调用 sleep 系统调用
    phi_take_forks_sema(i);
    // 需要两只叉子，或者阻塞
    cprintf("Iter %d, No.%d philosopher_sema is eating\n", iter, i); /* 进餐 */
    do_sleep(SLEEP_TIME);  // 模拟进餐，调用 sleep 系统调用
    // 把两把叉子同时放回桌子
