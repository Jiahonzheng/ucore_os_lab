...
void proc_run(struct proc_struct *proc) {
  if (proc != current) {
    ...
    local_intr_save(intr_flag);  // 关闭中断
    {
      ...
      load_esp0(next->kstack + KSTACKSIZE);  // 修改 esp 寄存器
      lcr3(next->cr3);                       // 载入页目录表地址
...
int kernel_thread(int (*fn)(void *), void *arg, uint32_t clone_flags) {
  ...
  tf.tf_cs = KERNEL_CS;                        // 设置代码段
  tf.tf_ds = tf.tf_es = tf.tf_ss = KERNEL_DS;  // 设置数据段
  tf.tf_regs.reg_ebx = (uint32_t)fn;   // 设置线程函数fn的实际地址
  tf.tf_regs.reg_edx = (uint32_t)arg;  // 设置线程函数fn的参数
  tf.tf_eip = (uint32_t)kernel_thread_entry;  // 设置eip寄存器
  return do_fork(clone_flags | CLONE_VM, 0, &tf);
}

// setup_kstack - alloc pages with size KSTACKPAGE as process kernel stack
static int setup_kstack(struct proc_struct *proc) {
  struct Page *page = alloc_pages(KSTACKPAGE);  // 分配内存页
  if (page != NULL) {
...
static void copy_thread(struct proc_struct *proc, uintptr_t esp,
  ...
  *(proc->tf) = *tf;
  ...
  proc->context.eip = (uintptr_t)forkret;     // 设置eip寄存器
  proc->context.esp = (uintptr_t)(proc->tf);  // 设置esp寄存器
}
...
void proc_init(void) {
  ...
  idleproc->pid = 0;                        // 设置进程 PID
  idleproc->state = PROC_RUNNABLE;          // 设置进程状态
  idleproc->kstack = (uintptr_t)bootstack;  // 设置进程内核栈
  idleproc->need_resched = 1;               // 设置进程为可调度
  set_proc_name(idleproc, "idle");          // 设置进程名称
  nr_process++;                             // 更新计数器
  ...
  set_proc_name(initproc, "init");  // 设置进程名称
